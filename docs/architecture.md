...
# System Overview
## Chat Widget (Browser): Custom TS widget embedded via <script> tag on static HTML site 
## FastAPI Server: Core backend logic handler 
    ...
    - logging service 
## External Dependencies: OAI API for LLM and voice models 
## Database: PostgreSQL (conversation + logging ) + pgvector (vector embeddings) + alembic + sqlalchemy
## Deployment: Railway 
